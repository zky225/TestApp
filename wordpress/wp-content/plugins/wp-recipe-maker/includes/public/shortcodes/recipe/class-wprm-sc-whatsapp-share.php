 ...
 */
class WPRM_SC_Whatsapp_Share extends WPRM_Template_Shortcode {
	public static $shortcode = 'wprm-recipe-whatsapp-share';
	...
	public static function init() {
		self::$attributes = array(
			...
			'text' => array(
				'default' => __( 'Share with WhatsApp', 'wp-recipe-maker' ),
				'type' => 'text',
			),
			'text_style' => array(
				'default' => 'normal',
	...
	public static function shortcode( $atts ) {
		...
		$whatsapp_url = 'https://wa.me/?text=' . rawurlencode( $body );
		...
		if ( $atts['icon'] ) {
			...
			if ( $icon ) {
				$icon = '<span class="wprm-recipe-icon wprm-recipe-whatsapp-share-icon">' . $icon . '</span> ';
			}
		}
		...
		if ( 'text' !== $atts['style'] ) {
			$classes[] = 'wprm-recipe-whatsapp-share-' . $atts['style'];
		...
		$aria_label = '';
		if ( ! $text ) {
			$aria_label = ' aria-label="' . __( 'Share with WhatsApp', 'wp-recipe-maker' ) . '"';
		}

		$output = '<a href="' . esc_attr( $whatsapp_url ) . '" data-recipe="' . esc_attr( $recipe->id() ) . '" style="' . esc_attr( $style ) . '" class="' . esc_attr( implode( ' ', $classes ) ) . '" target="_blank" rel="nofollow"' . $aria_label . '>' . $icon . WPRM_Shortcode_Helper::sanitize_html( $text ) . '</a>';
		return apply_filters( parent::get_hook(), $output, $atts, $recipe );
	}
}

WPRM_SC_Whatsapp_Share::init();